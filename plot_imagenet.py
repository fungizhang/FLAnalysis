...
for i in range(1):
    filename="C:/Users/Fungi/Desktop/imagenet/tmp.csv"
    ...
    with open(filename,'r')as file:
        ...
        for row in reader:
            ...
            print("row",row)
            ### attack success rate
            fedavg.append(float(row[0])/100)
            krum.append(float(row[1])/100)
            multi_krum.append(float(row[2])/100)
            xmam.append(float(row[3])/100)

            # ## testing error rate
            # fedavg.append(1-float(row[0])/100)
            # krum.append(1-float(row[1])/100)
            # multi_krum.append(1-float(row[2])/100)
            # xmam.append(1-float(row[3])/100)
    ...
    aaaa = "imagenet_backdoor_noattack"
    ...
    # plt.legend(labels=["FedAvg*", "FedAvg", "Krum", "Multi-Krum", "NDC", "RFA", "RSA", "XMAM"], fontsize=15)
    plt.legend(labels=["FedAvg", "Krum", "Multi-Krum", "XMAM"])
